function loadTransactionDetails() {
	...
	const totalPrice = calculateTotalPrice(
		transactionData.paperSize,
		transactionData.pagesPrinted
	);
	...
	document.querySelector(".transaction-details").innerHTML = `
        <div class="print-detail">
            <table>
                <tbody>
                    <tr>
                        <th>Mã đơn in: <span class="history-detail">${truncateString(
							transactionData._id
						)}</span></th>
                        <th>Thời gian: <span class="history-detail">${
							transactionData.time
						} - ${formatDate(
		transactionData.createdAt.slice(0, 10)
	)}</span></th>
                    </tr>
                    <tr>
                        <th>Mã máy in: <span class="history-detail">${
							transactionData.printerCode
						}</span></th>
                        <th>Khổ giấy: <span class="history-detail">${
							transactionData.paperSize
						} - ${transactionData.orientation}</span></th>
                    </tr>
                    <tr>
                        <th>Số tờ A4 sử dụng: <span class="history-detail">${
							transactionData.pagesPrinted
						}</span></th>
                        <th>Tên file: <span class="history-detail" id="fileName">${
